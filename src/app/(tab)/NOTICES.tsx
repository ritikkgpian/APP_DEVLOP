...
const noticeData=require("../../../NOTICES.json")
const dispatch=useDispatch();
 const tabs=["Academics","Placements and Internships","Hostel and Mess","Gymkhana and Clubs"]
  ...
  const handleBrowserClick= async ()=>{
    setLoading(true);
    setError(null);
    // if it is loading there should be no error 
     try{
  ...
  // we have to use ?. not .filter 

  const filteredNotices=notices?.filter(
    (item:any)=>{
       const categoryFilter=item.category==category;
       const searchFilter=
         (item.title.toLowerCase()|| item.shortContent.toLowerCase()).includes(query.toLowerCase())
    ...
    return(
        <ScrollView style={{backgroundColor:"wheat",height:"100%"}} >
           <View style={{marginLeft:40,marginTop:20}} >
            <TextInput   onChangeText={(e)=>setQuery(e)} placeholder="Search any notice..."  placeholderTextColor="white" 
            ...
            ></TextInput>
                    ...
                    tabs.map(
                        (item,index)=>{
                           const isActive=item==category;
                            return(
                                <TouchableOpacity  key={index}   onPress={()=>setCategory(item)} style={{width:150,borderWidth:3,backgroundColor:isActive ? "orange" : "red"}} >
             ...
             {
                loading && (
                    <Text style={{fontFamily:"sans-serif",fontWeight:800,color:"black",textAlign:"center",fontSize:20,marginTop:15}} >Fetching Notices......</Text>
                )
             }
             {
                error && (
             ...
             {!loading && notices && (
                <View style={{flexDirection:"row",flexWrap:"wrap"}} >
                    ...
                    <Text style={{fontFamily:"sans-serif",fontWeight:800}} >Sorry!! No such notices found.</Text>
                    ):(
                     filteredNotices.map(
                        (item:any,index:number)=>{
                            return(
                                ...
                                <View   key={index} style={{height:240,width:250,borderWidth:2,borderColor:"black",backgroundColor:"white",flexDirection:"column",gap:17,marginLeft:20,marginTop:10}} >
                                   <Text style={{fontFamily:"sans-serif",fontWeight:800,textAlign:"center",color:"blue"}} >NOTICE</Text>
                  ...
                  <Modal  visible={selectedItem!=null} >
                    {selectedItem && (
                                        <View style={{height:600,width:350,flexDirection:"column",gap:10,marginTop:80,marginLeft:20,backgroundColor:"aqua",borderWidth:3,borderRadius:20}} >
                                        <Text style={{fontFamily:"sans-serif",fontWeight:800,textAlign:"center",color:"blue"}} >NOTICE</Text>
